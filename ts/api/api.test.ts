...
describe("ChannelName API", () => {
    ...
    it("Should properly reject if status code is 401", () => {
        ...
        expect(API.getChannelName("1")).rejects.toEqual({ state: States.NOTAUTHORIZED });
    });
});
...
describe("API auth", () => {
    ...
    it("Should properly reject if status code is 401", () => {
        fetchMock.get("*", { status: 401 });
        expect(API.auth("1")).rejects.toEqual({ state: States.NOTAUTHORIZED });
    });
    it("Should properly reject if network error thrown", () => {
        fetchMock.get("*", { throws: { message: "network error" } });
        expect(API.auth("1")).rejects.toEqual({ state: States.OFFLINE });
    });
});
...
describe("UserInfo API", () => {
    ...
    it("Should properly reject if status code is 401", () => {
        fetchMock.get("*", { status: 401 });
        expect(API.getUserInfo()).rejects.toEqual({ state: States.NOTAUTHORIZED });
    });

    it("Should properly reject if status code is 403", () => {
        fetchMock.get("*", { status: 403 });
        expect(API.getUserInfo()).rejects.toEqual({ state: States.FORBIDDEN });
    });

    it("Should properly reject if status code is 404", () => {
        fetchMock.get("*", { status: 404 });
        expect(API.getUserInfo()).rejects.toEqual({ state: States.NOTFOUND });
    });
});
